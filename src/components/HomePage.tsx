        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <div 
                key={index}
                className={`text-center p-6 rounded-xl transform transition-all duration-500 hover:scale-105 ${
                  isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                } ${stat.color === 'blue' ? 'bg-blue-50 hover:bg-blue-100' : 'bg-yellow-50 hover:bg-yellow-100'}`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <div className={`inline-flex p-3 rounded-full mb-4 ${
                  stat.color === 'blue' ? 'bg-blue-500' : 'bg-yellow-500'
                }`}>
                  <stat.icon className="w-8 h-8 text-white" />
                </div>
                <div className={`text-3xl font-bold mb-2 ${
                  stat.color === 'blue' ? 'text-blue-900' : 'text-yellow-700'
                }`}>
                  {stat.value}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {departments.map((department, index) => {
              ...
              return (
                <div
                  key={department.id}
                  className={`group bg-white rounded-xl shadow-lg hover:shadow-2xl transform transition-all duration-500 hover:scale-105 cursor-pointer overflow-hidden ${
                    isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  }`}
