  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        ...
        // Fetch service data
        const serviceResponse = await fetch(`http://localhost:4000/api/services/${serviceId}`);
        
        if (!serviceResponse.ok) {
          if (serviceResponse.status === 404) {
            throw new Error(`Service with ID ${serviceId} not found`);
          }
          throw new Error(`Failed to fetch service: ${serviceResponse.status} ${serviceResponse.statusText}`);
        }
        ...
        // Fetch services list for navigation
        const servicesResponse = await fetch('http://localhost:4000/api/services/active/list');
        if (servicesResponse.ok) {
          const servicesData = await servicesResponse.json();
          setServices(servicesData);
        }
        
        // Fetch departments for navigation using your existing API endpoint
        const departmentsResponse = await fetch('http://localhost:4000/api/departments/all');
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-2">{error || 'Service not found'}</p>
            <p className="text-gray-500 text-sm mb-6">Service ID: {serviceId}</p>
            <button
              onClick={onBack}
            ...
            <button
              ...
              className="ml-4 bg-gray-200 text-gray-700 px-6 py-3 rounded-lg hover:bg-gray-300 transition-colors duration-300"
            >
              Try Again
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="prose max-w-none">
                ...
                {steps.length > 0 ? (
                  ...
                  <div className="mt-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-gray-600 text-center">No step-by-step process available for this service.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            ...
            {/* Additional Info */}
            <div
              className={`bg-white rounded-xl shadow-lg p-6 mt-6 transform transition-all duration-1000 ${
                isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-1 text-sm font-medium rounded ${getCategoryColor(service.category)}`}>
                    {service.category}
                  </span>
                </div>
                <div className="flex justify-between items-center">
