...
const Navigation: React.FC<NavigationProps> = ({
  ...
  const handleMobileNavClick = (viewId: string) => {
    if (viewId === 'home') {
      ...
      onNavigate(viewId as 'home' | 'manual');
    }
    setIsMobileMenuOpen(false);
  };

  const handleFilterSelect = (filter: 'all' | 'newcomer' | 'senior') => {
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center px-3 py-2 lg:px-4 lg:py-2 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 ${isActive
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center w-full px-4 py-3 text-left transition-colors duration-300 ${isActive
